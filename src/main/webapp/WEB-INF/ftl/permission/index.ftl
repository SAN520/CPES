    ...
    <div class="row">
        ...
        <article id="treeArt" class="col-sm-12 col-md-12 col-lg-12">
            <div class="jarviswidget jarviswidget-color-darken" data-widget-togglebutton="true" id="wid-id-1" data-widget-deletebutton="false" data-widget-editbutton="false">
                <header>
                    <h2><i class="fa fa-sitemap"></i>统菜单列表 </h2>
                </header>
                <div>
...
<script>
	...
	loadScript("${APP_PATH}/js/plugin/ztree/jquery.ztree.all-3.5.min.js",function(){
		var setting = {
					...
					async: {
						enable: true,
						url:"${APP_PATH}/system/permission/tree.htm",
...
// 					}
				...
				function addHoverDom(treeId, treeNode) {
					...
					if ( treeNode.level == 0 ) {
						s += '<a class="btn btn-info dropdown-toggle btn-xs" style="margin-left:10px;padding-top:0px;" href="${APP_PATH}/system/permission/add.htm?level='+treeNode.level+'&pid='+treeNode.id+'" data-toggle="modal" data-target="#remoteModal">&nbsp;&nbsp;<i class="fa fa-fw fa-plus rbg "></i></a>';
					} else if ( treeNode.level == 1 ) {
						s += '<a class="btn btn-info dropdown-toggle btn-xs" style="margin-left:10px;padding-top:0px;" href="${APP_PATH}/system/permission/edit.htm?level='+treeNode.level+'&id='+treeNode.id+'" data-toggle="modal" data-target="#remoteModal" title="修改权限信息">&nbsp;&nbsp;<i class="fa fa-fw fa-edit rbg "></i></a>';
	     ...
	     layer.confirm("是否删除【"+name+"】许可信息",  {icon: 3, title:'提示'}, function(cindex){
	      ...
	      $.ajax({
	          ...
	          success : function(respData){
	                        if(respData.success){
	                             layer.msg("许可删除成功", {time:2000, icon:1}, function(){
